...
def load_canon_character(campaign_id: str, name: str = "", kind: str = "npc", add_to_party: bool = False,
    ...
    with campaign_lock(campaign_id):
        ...
        # DEDUP (B-MED-1 + #940): match by exact canonical name across ALL kinds first (a
        # player/monster already bearing this name still blocks a duplicate — today's behavior),
        # THEN fall back to the roster matcher so a canon figure seeded under a FULLER display
        # name is promoted in place instead of fresh-loaded as a duplicate: canon "Wyll" ->
        # rostered "Wyll Ravengard" (npc-wyll), canon "Minsc" -> "Minsc and Boo" (npc-minsc).
        # _find_existing_roster_match only considers npc/companion roster records (never a
        # player/monster) and keys on the resolved canonical name, so this is STRICTLY ADDITIVE —
        # it can only ADD matches the exact-name check missed, never remove one. Empirically the
        # only new matches across the BG corpus (2076 canon files) are Wyll and Minsc, both
        # correct same-figure dedups; the rostered record then flows through the dossier merge
        # below and the documented load -> recruit_companion seating.
