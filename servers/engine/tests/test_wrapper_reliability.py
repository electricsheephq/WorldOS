...
* F12-1 (#753-adjacent): the flat 200s routine-beat deadline killed ~18% of HEALTHY beats
  ...
  Fix: routine default 200 -> 360 in ``worldos_dm_timeout`` + a ``worldos_dm_retry_timeout``
...
* F12-3 (#777): play.sh's cold open recorded an EMPTY opening unconditionally and entered the
  ...
  Fix: the empty-DMSG abort + the ``worldos_pc_seated`` seating guard (factored to the lib,
...
def test_play_lanes_invoke_the_dm_through_the_shim():
    ...
    for name in ("scripts/play.sh", "scripts/play_party.sh"):
        ...
        # and worldos_dm_retry_timeout end in the same token, hence the word-boundary lookbehind).
...
def test_routine_beat_timeout_default_is_360():
    ...
    r = _bash(_CLEAN + 'echo "bt=$(worldos_dm_timeout 0)"')
    assert r.returncode == 0, r.stderr
...
def test_play_lanes_recompute_the_retry_deadline():
    ...
    the retry (the F12-1 reuse bug) — they must now recompute via worldos_dm_retry_timeout."""
    for name in ("scripts/play.sh", "scripts/play_party.sh"):
        src = _src(name)
        assert 'beat_timeout="$(worldos_dm_retry_timeout "$beat_timeout")"' in src, (
...
def test_pc_seated_helper_matches_the_viewer_contract(tmp_path):
    """worldos_pc_seated == viewer _action_actor: seated means a party member whose character
    ...
    script = (
        f'set -u; . "{LIB}"\n'
        f'worldos_pc_seated "{sd}" c-seated; echo "seated=$?"\n'
        f'worldos_pc_seated "{sd}" c-npc-only; echo "npc=$?"\n'
        f'worldos_pc_seated "{sd}" c-pc-not-in-party; echo "unparty=$?"\n'
        f'worldos_pc_seated "{sd}" c-missing; echo "missing=$?"\n'
        f'worldos_pc_seated "{sd}" ""; echo "blank=$?"\n'
    )
    r = _bash(script)
    assert r.returncode == 0, r.stderr
...
def test_play_sh_runs_the_seating_guard_with_one_reseat_then_loud_abort():
    """play.sh must guard seating like play_party: worldos_pc_seated, ONE reseat retry on a fresh
    ...
    play = _src("scripts/play.sh")
    assert play.count("worldos_pc_seated") >= 2, "guard must re-check after the reseat retry"
    assert "COLD-OPEN SEATED NO PC" in play, "the second miss must abort loudly"
    guard_at = play.index("worldos_pc_seated")
    loop_at = play.index("while true; do")
    ...
    reseat_at = play.index("worldos_pc_seated")
...
def test_play_party_reuses_the_lib_seating_guard():
    ...
    assert "worldos_pc_seated" in party, "play_party.sh must call the shared seating guard"
    assert "pc_seated()" not in party, "the local pc_seated() must be deleted (lib is the one impl)"
    assert party.count('worldos_pc_seated "$STATE_DIR" "$CAMPAIGN_ID"') >= 2, (
...
def test_pc_seated_lives_in_the_lib():
    ...
    assert "worldos_pc_seated()" in lib, "the seating guard must be factored into the shared lib"
...
def test_play_party_emits_the_wrapper_heartbeat_on_the_cold_open():
    ...
    party = _src("scripts/play_party.sh")
    assert "worldos_emit_progress_heartbeat" in party, "play_party.sh must emit the wrapper heartbeat"
    hb_at = party.index('worldos_emit_progress_heartbeat "$CAMPAIGN_ID" 1 0')
...
def test_play_party_emits_the_wrapper_heartbeat_before_companion_moves():
    ...
    party = _src("scripts/play_party.sh")
    beat_hb_at = party.index('worldos_emit_progress_heartbeat "$CAMPAIGN_ID" 0')
...
def test_play_party_soft_ticks_after_each_recorded_beat():
    """Mirror of play.sh:475-478/504: capture PREV_DAY/PREV_TOD pre-beat, worldos_soft_tick after
    record_dm_reply — play_party was the ONLY beat loop without the backstop (frozen day-1 morning)."""
    party = _src("scripts/play_party.sh")
    assert 'worldos_soft_tick "$ROOT" "$STATE_DIR" "$PREV_DAY" "$PREV_TOD"' in party, (
    ...
    tick_at = party.index('worldos_soft_tick')
