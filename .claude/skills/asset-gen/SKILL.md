...
- **`tripo_gen.py`** — `text|image|rig` subcommands; `--lowpoly` (P1); `--test-key`; `--dry-run` (credit est). Uses the **v2 OpenAPI** (`https://api.tripo3d.ai/v2/openapi`, single `POST /task` + `type` discriminator); polls `GET /task/{id}` ≥2s; **downloads GLB immediately (URLs expire ~5 min)**. Output → gitignored `content/worlds/_private/<world>/images/<scope>/`. (Anim presets: `idle|walk|run|dive|climb|jump|slash|shoot|hurt|fall|turn`; no `attack`/`cast` → wrapper aliases attack→slash, cast→shoot.)
...
- **Tripo (v2 OpenAPI — fixed 2026-06-28; old `openapi.tripo3d.ai/v3` is DEAD = 404 `code:4001`)**: Bearer, base **`https://api.tripo3d.ai/v2/openapi`**. **Every operation is `POST /task` with a `type` field** (no per-op paths): `text_to_model` `{prompt, model_version:"v3.1-20260211"}` (low-poly `"P1-20260311"`), `image_to_model` `{file:{type:"jpg",file_token}, model_version}`, then the rig chain `animate_prerigcheck` → `animate_rig` `{rig_type:"biped", spec:"mixamo", out_format:"glb"}` → `animate_retarget` `{animation|animations:[preset:*], out_format:"glb"}`. Presets: `idle|walk|run|dive|climb|jump|slash|shoot|hurt|fall|turn` (NO `attack`/`cast`). Poll `GET /task/{id}` ≥2s → `data.{status,progress,output}`; **model URL under `output.{pbr_model|model|base_model}`** (rig/retarget reuse the same keys); URLs expire ~5 min. Upload one image: `POST /upload` multipart → `data.image_token`. Balance `GET /user/balance`. Insufficient credit = `code:2010` (HTTP 403). Verified vs the official `tripo3d` PyPI SDK v0.4.1 + live probes.
