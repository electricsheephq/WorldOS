...
#
#   (A) qa/fast_gate.sh OPT-IN green-cache (WORLDOS_FASTGATE_CACHE=1):
...
#         WORLDOS_RAM_PREFLIGHT_STRICT=1, pointing at GitHub CI / the support VM.
...
# mocked via WORLDOS_FASTGATE_INNER_CMD, and available RAM is forced via WORLDOS_RAM_PREFLIGHT_AVAIL_MB.
...
run_fastgate() {  # runs fast_gate.sh with the mocked inner + redirected cache home
  WORLDOS_FASTGATE_CACHE=1 \
  WORLDOS_FASTGATE_INNER_CMD="$TMP/inner.sh" \
  WORLDOS_FASTGATE_CACHE_DIR="$CACHE_HOME" \
  WORLDOS_FASTGATE_PERSONA="velocity-test" \
    bash "$ROOT/qa/fast_gate.sh" >"$TMP/fg.out" 2>&1
}

# A0: default OFF — inner gate runs every time, no cache dir created.
: > "$COUNTER"; mk_inner 0
WORLDOS_FASTGATE_INNER_CMD="$TMP/inner.sh" WORLDOS_FASTGATE_CACHE_DIR="$TMP/cache_off" \
...
# "available RAM" reading via WORLDOS_RAM_PREFLIGHT_AVAIL_MB. We don't care whether the OTHER
...
run_preflight() {  # $1 = forced avail MB ; $2..= extra env assignments (e.g. STRICT=1)
  local avail="$1"; shift
  env "$@" WORLDOS_RAM_PREFLIGHT_AVAIL_MB="$avail" \
...
run_preflight 800 WORLDOS_RAM_PREFLIGHT_STRICT=1; rc_strict=$?
...
run_preflight 64000 WORLDOS_RAM_PREFLIGHT_STRICT=1 >/dev/null 2>&1 || true  # may fail later on art/port
