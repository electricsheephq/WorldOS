...
def main() -> int:
    ...
    # The {{BENCH CARD}} token is OPTIONAL (#1040 scorer-opt). The shrunk rubric no longer inlines
    # the verbose SRD rule text — the engine enforces those rules and the behavioral gate enforces
    # the deterministic 5e checks, so the lens grades only DM-owned omissions + hallucinated
    # numbers. The card file (angry_dm_card.md) is STILL generated above (a regenerable SRD
    # reference + the freshness guard against an SRD bump), it just isn't injected when the token is
    # absent. When the token IS present, behavior is unchanged (back-compat).
    has_token = BENCH_CARD_TOKEN in src
    ...
    src = re.sub(
        ...
        r"Do NOT hand-edit: edit the .src.md and rerun the build. -->\n\n",
        src,
        count=1,
        flags=re.DOTALL,
    )
    rubric = src.replace(BENCH_CARD_TOKEN, card.rstrip()) if has_token else src
    RUBRIC_OUT.write_text(rubric, encoding="utf-8")
    if not has_token:
        print(
            f"[build_angry_dm_card] note: {BENCH_CARD_TOKEN!r} absent from the src — rubric written "
            f"WITHOUT the inlined SRD bench card (the lean #1040 lens); {CARD_OUT.relative_to(ROOT)} "
            f"still regenerated as the standalone SRD reference.",
            file=sys.stderr,
        )
