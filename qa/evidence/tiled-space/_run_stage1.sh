#!/bin/bash
# Stage-1 generation for the TILED-SPACE spike. Run from repo root.
set -e
cd "$(dirname "$0")/../../.."
...
STYLE="moody hand-painted painterly environment art, Pillars of Eternity Deadfire isometric backdrop, dramatic chiaroscuro, warm firelight vs cool teal shadow, dark earthy palette, dimetric bird's-eye view, no characters, no text"
mkdir -p "$PLATES"

echo "=== [1/4] ARM A wide 2-room @2048 ==="
python3 "$SG" controlnet --model-id "$FLUX" --control-modality depth --control-strength 0.72 \
...
echo "=== [2/4] ARM A escalation 3-room @2048 (0.67x density) ==="
python3 "$SG" controlnet --model-id "$FLUX" --control-modality depth --control-strength 0.72 \
...
echo "=== [3/4] Baseline single-room + ARM B tile1 (tileL control) @1024 ==="
python3 "$SG" controlnet --model-id "$FLUX" --control-modality depth --control-strength 0.72 \
...
echo "=== [4/4] ARM B negative control — naive tile2 (no conditioning) @1024 ==="
python3 "$SG" controlnet --model-id "$FLUX" --control-modality depth --control-strength 0.72 \
