...
class MacOSAppStaticContractTests(unittest.TestCase):
    ...
    def test_built_app_playtest_can_keep_minted_backend_for_manual_gameplay(self):
        ...
        self.assertIn('defaults write dev.worldos.app selectedProvider "$SELECTED_PROVIDER"', harness)
        self.assertIn("requires WOS_APP_PART=A", harness)
        self.assertIn('KEEP_MINTED_BACKEND="${WOS_APP_KEEP_MINTED_BACKEND:-0}"', harness)
        self.assertIn('NATIVE_AUTOSTART="${WOS_APP_NATIVE_AUTOSTART:-0}"', harness)
        self.assertIn("native-launcher-state", harness)
        self.assertIn("seed_native_launcher_state() {", harness)
        self.assertIn("launcher-state-seed.json", harness)
        self.assertIn("qaAutoStartProvider", harness)
        self.assertIn("qaAutoStartWorld", harness)
        self.assertIn("qaAutoStartRunID", harness)
        self.assertIn('CODEX_HOME_FOR_APP="${WOS_APP_CODEX_HOME:-${CODEX_HOME:-}}"', harness)
        self.assertIn("ORIGINAL_CODEX_HOME", harness)
        self.assertIn('restore_app_default codexHome "$ORIGINAL_CODEX_HOME"', harness)
        self.assertIn('defaults write dev.worldos.app codexHome "$CODEX_HOME_FOR_APP"', harness)
        self.assertIn("native QA auto-start is driving startProviderSession", harness)
        self.assertIn("kill_repo_viewers() {", harness)
        self.assertIn("port_matches_run() {", harness)
        self.assertIn("(.live.run_id // \"\") == $run", harness)
        self.assertIn('endswith("/play-state/" + $run)', harness)
        self.assertIn('port_matches_run "$p" "$new_dir"', harness)
    ...
    def test_model_agnostic_provider_config_preserves_anthropic_and_codex_defaults(self):
        ...
        self.assertIn('@AppStorage("codexHome")', root)
        ...
        self.assertIn("Codex home", settings)
        self.assertIn("optionalAbsolutePathError", settings)
        self.assertIn("QA player model", settings)
        self.assertIn("QA scorer model", settings)
        self.assertIn("TextField(\"Codex home\"", provider_view)
        ...
        self.assertIn("let codexHome: String", models)
        self.assertIn('env["CODEX_HOME"]', providers)

    def test_native_app_has_one_shot_qa_autostart_hook(self):
        root = self.read("macos/WorldOSApp/Sources/WorldOSApp/Views/RootView.swift")
        handoff = self.read("qa/app_handoff_gate.py")

        self.assertIn('@AppStorage("qaAutoStartProvider")', root)
        self.assertIn('@AppStorage("qaAutoStartWorld")', root)
        self.assertIn('@AppStorage("qaAutoStartRunID")', root)
        self.assertIn("startQAAutoProviderIfRequested()", root)
        self.assertIn('qaAutoStartProvider = ""', root)
        self.assertIn('qaAutoStartWorld = ""', root)
        self.assertIn('qaAutoStartRunID = ""', root)
        self.assertIn("processService.startProviderSession(", root)
        self.assertIn("processService.markProviderViewerReady()", root)
        self.assertIn('"WOS_APP_NATIVE_AUTOSTART": "1"', handoff)
    ...
    def test_codex_scorer_is_drop_in_and_never_invokes_claude(self):
        ...
        self.assertIn("validate_codex_service_tier", score_codex)
        self.assertIn("Codex CLI config drift", score_codex)
        self.assertIn("service_tier must be unset, 'fast', or 'flex'", score_codex)
