...
its re-styled sarcophagus repaints noticeably LARGER (now cols 4-9 x rows 3-7, up from the
...
relocated to (11,3)/(2,7), clear of the widened footprint.
...
def test_sarcophagus_footprint_is_impassable():
    ...
    crypt_armb_iter3_v1.png plate — cols 4-9 x rows 3-7 — must be an engine pathing obstacle, so
    ...
    assert sg.SARCOPHAGUS_CELLS == [[c, r] for c in range(4, 10) for r in range(3, 8)]
...
def test_hero_and_goblin_spawn_cells_stay_walkable():
    """The hero(11,3)/goblin(2,7) spawn cells (relocated #1386 off the widened sarcophagus
