...
| packaged pins | `uv run --directory servers/engine python qa/packaged_pins.py <WorldOSPlayer.app>` | free; before every sandbox gate, install, and closeout | JSON report with per-room parity and GREEN/RED/ERROR verdict | — (gate; no scores row) | ROOM-PIPELINE-RUNBOOK step 10; #1651 |
