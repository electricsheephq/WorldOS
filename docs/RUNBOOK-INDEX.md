...
| agent-play DM loop (DM-only, live sandbox) | `qa/agent_play.sh start\|say\|serve\|status\|stop` against an ALREADY-RUNNING `qa/qa_sandbox.py up` stack (`--engine <engine-port>` + `--state <state dir>`); `--dry-run` exercises everything but the `claude -p` call | ~$0.5-1 per beat (opus DM, one beat per `say`/player line); the PLAYER is an agent or a human — no second `claude -p` | the run's `session.json` + `<run>.quest_trace.json` + `<run>.dm.*.jsonl` under `qa/agent_play_runs/<run>/`, and the two-sided rows in `<state>/chat.jsonl` the viewer play screen renders | — (no scores_db row: this is an interactive lane, not a scored eval — score a transcript with the run_adventure/duo lenses if you need one) | worldos-dev; shares `qa/lib_adventure_dm.sh` with `qa/run_adventure.sh` (same brief/hermetic env/retry/quest stamping). MOVEMENT is NOT in this runner — walk on the QA channel `/click` or the viewer `/move`. `serve` is what the owner instance's `org.worldos.owner-dm` LaunchAgent runs |
