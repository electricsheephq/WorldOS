...
| qa sandbox lane | `qa/qa_sandbox.py up …` / `down --run <id>` (see module docstring) | free; second engine :8866 + QA channel :8972 (owner instance = 8776/8981; ⚠ 8766 is the claude-max bridge, never a WorldOS port) | sandbox meta (ports + OWN pids); ⚠ #1667: VERIFY the serving pid before citing any gate run as evidence | — (infrastructure for every live gate) | ROOM-PIPELINE-RUNBOOK §10b; #1596 / #1667 |
| player_cert (G1 live slice) | `qa/player_cert.py --live …` against the sandbox stack (CI half in ci.yml) | free; needs a built player in the sandbox | tri-state GREEN/RED/ERROR (exit 0/1/2) + per-assertion report incl. the build stamp — a harness defect is ERROR, never a verdict | — (gate; roster-complete suite = #1651) | charter #1651; OPERATIONS install gate; PRODUCT-ROADMAP §9 G1 |
| arc-duo TEXT eval (A-T, §9 G2) | `qa/run_adventure.sh <run>` → `qa/quest_progress.py` stamps → `qa/adventure_eval.py` | ~$4-6 / run (opus DM, ~45 min, SOLO tenant — never beside adventure_walk's VQA); N≥3 blind-adjudicated for a citable verdict | `qa/transcripts/<run>.*` + `<run>.quest_trace.json`; adventure_eval row | scores_db surface=adventure (`av_` ruler) | PRODUCT-ROADMAP §4d; needs a live `claude -p` credential (CLAUDE_CODE_OAUTH_TOKEN or keychain login — an expired OAuth aborts the run at the player intro) |
| walked-arc eval (A-G, §9 G3) | `qa/adventure_walk.py` over the FULL route camp→tavern_snug→crypt→throne_hall→camp on the sandbox stack | ~$1-2 (per-stage VQA); needs a built player + sandbox | `adventure_walk_report.json` + per-stage frames (tri-state) | same aggregator, modality=walked | PRODUCT-ROADMAP §4d; PANEL-PROTOCOL (blind adjudication) |
