...
| qa sandbox lane | `qa/qa_sandbox.py up …` / `down --run <id>` (see module docstring) | free; second engine :8866 + QA channel :8972. ⚠ Runner DEFAULTS still name the pre-migration owner ports (`qa_sandbox.py` 8766, `walk_test.py` 8766/8971); on the Paris Mac 8766 is the claude-max bridge, so ALWAYS pass `--engine`/`--qa` explicitly (owner instance moves to 8776/8981 per #1702) | sandbox meta (ports + OWN pids); ⚠ #1667: VERIFY the serving pid before citing any gate run as evidence | — (infrastructure for every live gate) | ROOM-PIPELINE-RUNBOOK §10b; #1596 / #1667 |
