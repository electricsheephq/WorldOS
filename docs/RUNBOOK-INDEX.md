...
| agent-play DM loop (DM-only, live sandbox) | `qa/agent_play.sh start\|say\|serve\|status\|stop` against an ALREADY-RUNNING `qa/qa_sandbox.py up` stack (`--engine <engine-port>` + `--state <state dir>`); `--dry-run` exercises everything but the `claude -p` call | ~$0.5-1 per beat (opus DM, one beat per `say`/player line/move intent); the PLAYER is an agent or a human — no second `claude -p` | `session.json` + `<run>.quest_trace.json` + `<run>.dm.*.jsonl` + bounded `<run-dir>/closeout.json` (beats, stamps, spend, quest, chat), and the two-sided `<state>/chat.jsonl` rows | `<run-dir>/closeout.json` only; no scores_db row (the scored eval owns that) | worldos-dev; shares `qa/lib_adventure_dm.sh` with `qa/run_adventure.sh`; `serve` consumes viewer chat and `$WORLDOS_PLAYER_MOVES`, while engine-resolved `/move` kinds remain engine-owned. `stop` terminates the recorded serve PID and writes the closeout; the owner `org.worldos.owner-dm` LaunchAgent has `KeepAlive=false` |
| agent G4 two-pass playtest | pass 1 walkaround + pass 2 live-DM story; persist with `qa/agent_g4_row.py --build <sha> --p1 <n> --p2 <n> --p3 <n> --route-complete <0|1> --pass1 <PASS|FAIL> --pass2 <PASS|FAIL> --frames-dir <dir> --persist --run-id <id>` | $0 persistence; pass 2 follows agent-play cost | frames directory + triage/reproduction evidence + pass verdicts | agent_g4 | DESIGN-MEMO-2026-09-02 §1 · charter #1702 |
...
| occluder sidecar vs grid | `python3 qa/sidecar_grid_check.py` (every manifest sidecar) / `--room R --boxes candidate.json` (a fresh Editor export) | free, seconds, EVERY PR (ci.yml) + before promoting any `boxes/*.json` | stdout per room: `blocked-without-occluder` + `occluder-over-open-floor` cell lists; exit 0 only at 0/0 for every room | — (gate) | ROOM-PIPELINE-RUNBOOK §9 (sidecars) · #1793 Day 1 |
| owner installer dry-run | `qa/owner_install.sh dry-run <WorldOSPlayer.app> --stage <evidence-dir>` | free; mandatory before owner install/refresh handoff | three rendered plists (session/player/DM) + `install-ledger.json` with pins/kit-root/cert/`result=Succeeded` build identity and exact ports | — (gate; no scores row) | `docs/OWNER-INSTALL.md`; charter #1702 |
...
| walked-arc eval (A-G, §9 G3) | `qa/adventure_walk.py` on the sandbox stack — route: camp→tavern_snug→camp→crypt→throne_hall→crypt→camp→tavern_snug — it now ENDS AT THE GIVER (`return_to_giver`, Keeper Maera), whose `reward_leg` verdict is read from get_quests / quest_trace (needs `--state`, or `--run`'s sandbox.json) and sets the report's `route_complete`; the route is DATA (`--route` overrides the §9 DEFAULT_ROUTE) | ~$1-2 (per-stage VQA); needs a built player + sandbox | `adventure_walk_report.json` + per-stage frames (tri-state) | — (report JSON only; no scores_db persistence is wired for walked runs) | PRODUCT-ROADMAP §4d; PANEL-PROTOCOL (blind adjudication); gaps tracked in #1709 |
