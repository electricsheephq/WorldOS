...
| owner installer dry-run | `qa/owner_install.sh dry-run <WorldOSPlayer.app> --stage <evidence-dir>` | free; mandatory before owner install/refresh handoff | two rendered plists + `install-ledger.json` with pins/kit-root/cert/build identity and exact ports | — (gate; no scores row) | `docs/OWNER-INSTALL.md`; charter #1702 |
