...
- **A-T — The text-arc eval** *(parallel with A-G)*: `qa/run_adventure.sh` (duo-derivative,
  ...
  Four of them have TEETH: under `WORLDOS_GATE_ARC` the behavioral gate turns `reroll_character`,
  `add_location`, a `spawn_monster` off the seed's species (read from the run's own seed manifest,
  never hard-coded), and an `end_combat` returning `warning_live_hostiles` into hard FAIL rows
  naming the beat. run_duo is untouched (it never sets the flag) and the adventure ruler is
