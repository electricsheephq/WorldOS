...
| T-02 | **Major** | Caelar portrait is a silhouette across PartyRow + hero card | `screen-table.jsx:439`, `chrome.jsx:225-235` | epic:portraits | When the PC is custom-created (`bindHero`), the portrait pipeline writes `portrait-<id>.png` to the served image store. Until then, silhouette is the correct fallback for a faceless custom PC. This is a wait-on-EPIC-A item. |
