...
| BE-04 | **Major** | ✅ DONE Persons + Lore tabs have no live read-model | `screen-bestiary.jsx:96-99` | epic:wire-prototypes | Either (a) wire `/persons-surface` + `/lore-surface` to project known canon NPCs + recorded lore, OR (b) hide the tabs and document the deferment. Don't ship dead tabs. — **Resolved via option (b)**: the Persons + Lore tab strip is removed (no engine projection exists for either); `tab` is pinned to "creatures"; downstream branches kept so reintroduction is render-only. |
...
| BE-06 | **Major** | ✅ DONE "Body" + "Tactics" + "Loot" + "Marginalia" prose sections always rendered but rarely populated | `screen-bestiary.jsx:269-327` | epic:wire-prototypes + epic:per-page-polish | Already gated on truthy ✓ — verify there's no `body=" "` whitespace issue. When all four are blank, show a small "Lore not yet recorded for this creature." — **Resolved**: added an all-blank fallback that renders "Lore not yet recorded for this creature." when Body/Tactics/Loot/Marginalia are all absent; trim() guards cover the whitespace-only `body=" "` case. |
| BE-07 | **Major** | ✅ DONE "Provenance" section heading rendered when contentOrigin === "authored" — should also surface for SRD with `[SRD 5.2]` badge | `screen-bestiary.jsx:288-296` | epic:per-page-polish | When `entry.contentOrigin === "srd"`, show a small "[SRD]" badge next to the entry name OR in the eyebrow. Discloses source consistently. — **Resolved**: SRD creatures now render a small "SRD" badge beside the entry name (driven by the read-model's `content_origin`); authored creatures keep crediting source/license below. |
