...
**★ LOCAL BUILD PRE-FLIGHT (measured gap — stale manifests shipped in Box Cycles 2 AND 3, 2026-07-15):**
...
The local Unity project (`/Users/m1/worldos-unity/`) carries its own copies of these data files, and
`BuildMacOSPlayer.EnsurePackaged` packages the local copies verbatim. A lane that stages only its
changed `.cs`/shader files ships whatever manifest the project happened to have. Before EVERY local
`BuildMacOSPlayer`, sync the renderer data files from the repo main being built against:
...
pre-kit chunky sidecars under a green stamp) → the Unity project root. The project copy is NEVER the source of truth; the repo is. (Both cycle regressions were caught by
...
### 9c. Live rooms — "the room is the scene" (Day 2-3, 2026-09-03)
A live room is the kit build of a room's walked geometry, textured with painted tileables through the `WorldOS/PainterlyRoom` shader, baked to a prefab and instantiated by the player under `LiveRoom_<id>`. The picture, the collision and the occlusion are the same meshes: the seg-registration gate on the live render reads 100 % by construction.
1. **Textures** (once per room class, ~16 CU per surface best-of-2): six seamless tileables under `Assets/painterly/textures/<room>/{floor,wall,pillar,tomb,rubble,wood}.png` (wrap Repeat). Kind → file is resolved from the GEOMETRY kind (`_propKindById`), placed names only as a fallback.
2. **Build + look**: Editor env `WORLDOS_ROOM_GEO / WORLDOS_KIT_ROOM_ID / WORLDOS_ROOM_TEXTURES` → `BuildRoomKit.BuildRoom()` → apply the room's knob set (materials are in-memory; the knob set is the record — `session-notes/.../day-2/record.knobs` for the crypt) → `Tools/WorldOS/Kit/Capture Kit Room` writes the contract frame + the flat `_score.png`.
3. **Gates before any bake**: `qa/registration_score.py check <_score.png> <geometry> --ortho <pin> --boxes <kit sidecar>` = 100 %; the blind two-anchor panel (`qa/panel_workflow.mjs`, control = the PoE2 frame, calibration ref = a shipped plate) within 0.5 of the ref. Hide any actor proxies before a score capture.
4. **Bake**: `WORLDOS_KIT_ROOM_ID=<id>` + `Tools/WorldOS/Kit/Bake Live Room Prefab` → `Assets/Resources/LiveRooms/<id>.prefab` (+ `<id>.json` = fire globals **and** the camera grade). It duplicates the QA root (never mutates it), drops `Cast`, renames every `KitRoom_*` → `LiveRoom_*`, and writes EVERY in-memory material to `LiveRooms/<id>/mat_NN_<name>.mat` — the kit builds its materials at runtime, so an unsaved one ships as a missing (magenta) material.
5. **Ship**: manifest entry `"liveRoom": "<id>"` + `"liveRoomMode": "visible"|"occluder"` (keep `plate`/`boxes`/`cameraPin`). `visible` = strategy B, the room is the picture and re-applies the baked grade. `occluder` = strategy D, the PLATE is the picture and the same prefab is re-materialled to `WorldOS/OccluderDepth` with its lights off, its floor renderers off and the sidecar box proxies skipped (one occluder per wall, never two). Then build; `qa/packaged_pins.py`; `strings level0 | grep -c KitRoom_` = 0 (the build now byte-scans `resources.assets`/`sharedassets*` too, so a dirty prefab reds the build); sandbox `walk_test --exhaustive --visual 4` + `player_cert --live` + `/debug` (`liveRoom`/`liveRoomMode`/`liveRoomRenderers`/`grade`/`fps`).
Traps: Unity demotes ForcePixel lights beyond `pixelLightCount` to vertex lights — the shader takes explicit fire globals and has no vertex-light term, so the bake captures those globals and a `PainterlyRoomLights` component re-arms them in the player; `CaptureRoom` hides everything outside the kit root; `GameObject.Find` skips inactive objects; the Beautify grade is EDITOR-ONLY scene state — the bake copies its values onto the prefab and `BuildMacOSPlayer` strips the component from the build scene copy, so the plate rooms stay ungraded; and **`BuildPipeline.BuildPlayer` raises Unity's own save-modified-scenes modal whenever the active scene is dirty, which deadlocks a remotely-driven editor** — bake first, then build from a clean scene (the build now refuses up front rather than hanging).
