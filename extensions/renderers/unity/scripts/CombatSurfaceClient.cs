...
public class CombatSurfaceClient : MonoBehaviour
{
    ...
    void Start()
    {
        // #1466 ROOT CAUSE of "player QA input does nothing": a macOS player with Run-In-Background
        // OFF PAUSES its player loop (Update, coroutines, input) whenever it is not the FOREGROUND app.
        // The no-hijack launch (#1456/#1458) never activates the window, so the player rendered one frame
        // then froze — every input path (OS-synthetic mouse AND the QA HTTP channel) failed not on
        // delivery but because the main loop wasn't running (the HTTP listener is a separate thread, so
        // /health answered while the surface poll + click drain sat frozen). Force it on at runtime so a
        // backgrounded QA/beauty player keeps polling the surface and draining injected clicks.
