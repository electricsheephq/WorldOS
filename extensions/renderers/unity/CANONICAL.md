...
| **SCALE to a world — 3 distinct rooms at the ~7 textured-greybox quality** | **MET + extended.** The textured-greybox lever (the ~7 standard) stamps out DISTINCT authored rooms, not re-skins, each one-source authored-pathing-aligned: (1) **crypt** — corner pillars + central sarcophagus, **7.07**; (2) **cathedral nave** — flanking columns + apse altar + stained-glass, **6.97**; (3) **throne hall** — columned aisle + a raised throne dais, ~7. Same `build_room_greybox.cs` + `generate_room.py` pipeline, distinct `scene_grid` per room. Each lifts its room to ~7 (vs the old carved-only ~5-6) and shares the same LoRA-bound cap (focal craft PoE2-grade, broad wall fields softer). | `qa/seed_gfx_combat.py` (crypt) + `seed_gfx_church.py` (nave) + `seed_gfx_bosshall.py` (throne hall) + `build_room_greybox.cs` + `generate_room.py` | `~/worldos-session-notes/renders/THREE_ROOMS_textured.png` + `crypt14_walledcourse_v1.png` + `church_textured_v1.png` + `bosshall_textured_v1.png` | ✅ **3 distinct rooms @ ~7** (≥8 = better LoRA, owner-deferred) |
