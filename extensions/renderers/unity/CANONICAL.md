...
| **Carved-greybox (M-A ceiling lever) — DIRECTIONAL, not yet ≥8** | Carved greybox geometry (flagstone grout, wall pilasters/cornice) IMPROVES over a flat greybox (`flat_vs_carved_painted.png` is a real jump; the harsh critic credits "real carved flagstone + fluted pilasters + arch prove the greybox lever"). **BUT a rigorous 2-run visual-critic panel scored 4.75 (crypt bg) / 5.0 (combat) / 6.1 (church) — washout-bound, NOT ≥8** (logged `qa/scores_db` surface=visual). The earlier "≥8" was an eyeball over-claim — corrected. Root cause: the 7.4 recipe RELIGHTS a *painterly* base at low strength; a GRAY carved greybox under-paints at low strength (gray/washout) and drifts at high strength. ≥8 path = a painterly carved base (2-stage) or a crisper LoRA — **owner DEPRIORITIZED chasing it** vs the system. | `build_room_greybox.cs` (carved floor grout + wall pilasters/cornice) | `~/worldos-session-notes/renders/flat_vs_carved_painted.png` (flat→carved jump) | ◐ **lever real, ≥8 NOT reached** (deprioritized) |
