...
Call `worldos-engine` `session_recap` for the campaign (the id in the arguments, or the most recent from `list_campaigns`). Read the returned "Previously on your adventure…" text aloud through `worldos-voice` using the narrator voice, then briefly note where the party stands right now — location, active quest, and party HP.
