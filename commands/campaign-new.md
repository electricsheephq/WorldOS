...
2. Call `worldos-engine` `start_adventure(adventure_id)` — it CREATES and seeds the campaign (locations / NPCs / the companion) in one call and returns its `campaign_id`. Do NOT call `create_campaign` first: `start_adventure` mints its own campaign, so calling both leaves an empty orphan campaign separate from the seeded one. Use the returned `campaign_id` for every following call.
...
4. Confirm the **AI companion** has joined the party (its own sheet + `voice_id`). Introduce it in-character through the **companion** skill, voiced via `worldos-voice`.
