...
3. Call `start_session` to begin this play session. It returns `previously_on` — a recap of the **previous** session. Read it aloud as **"Previously on your adventure…"** via `clawdnd-voice` (narrator voice). (On a brand-new campaign it returns the new-adventure message instead — just open the story.)
