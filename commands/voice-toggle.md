...
- **off / silent:** stop calling `worldos-voice` `speak`. Keep narrating and voicing characters in **text only**, still labeling each speaker so the player knows who's talking. Confirm voice is off.
- **on / voiced:** resume calling `worldos-voice` `speak(text, voice_id)` for narration, NPCs, and the companion in their own voices. If the active backend is `null`, tell the player to set `WORLDOS_TTS_BACKEND=kokoro` and restart the session for actual audio.
