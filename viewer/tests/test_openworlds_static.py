...
class OpenWorldsStaticRouteTests(unittest.TestCase):
    ...
    def test_app_status_alias_matches_primary_route(self):
        status, ctype, body = self._get("/__worldos/app-status.json")

        self.assertEqual(status, 200)
        self.assertIn("application/json", ctype)
        payload = json.loads(body.decode("utf-8"))
        self.assertEqual(payload["schema"], "worldos.app-status.v1")
        self.assertEqual(payload["endpoints"]["app_status"], "/app-status")
