...
def _repo_version() -> str:
    ...
    if env and env.strip():
        return env.strip()
    version_file = _HERE.parent / "VERSION"
    try:
...
def _app_status_payload(
    *,
    port: int,
    attached_campaign_id: str,
    viewed_campaign_id: str,
    transcript_path: str,
    chat_path: str,
) -> dict:
    """Machine-readable app/test harness truth for agents.

    This is a read-only probe over the same viewer facts the UI already uses. Harnesses can ask
    which campaign/run/provider/art root they are actually driving before spending model time or
    ...
    return {
        ...
        "viewer": {
            "port": int(port),
            "repo_root": _resolved(_HERE.parent),
