...
6. **Resolve via tools** — checks/attacks/rules through the engine. In **combat**, see `reference/combat.md` (companion turns via `companion_suggest_action`, the action economy, the turn loop, damage/saves). Reach the companion only through its tool boundary; never silently skip its turn or fold its lines into your narration.
