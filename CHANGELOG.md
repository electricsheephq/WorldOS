...
- **HV1 — per-artifact eval instruments (#1323, epic Act II Harvest Loop).** The harvest loop's
  ...
  **±1.2 noise law**. Thin read-only snapshot reader (`qa/artifact_snapshot_reader.py`) extracts
  quests/NPCs from existing campaigns; `qa/artifact_calibration_panel.py` runs one blind panel per class.
  Artifact ruler `ac_986d87bf235a`; engine-duo `sc_be859353df20` / `lc_c603a22aac3d` unchanged.
  Shared envelope `data/library/artifact_schema.json` authored here (the HV2 extractor lane #1324
  rebases on it).
